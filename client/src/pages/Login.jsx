    ...
    <div className="max-w-md mx-auto mt-10 bg-white p-8 border border-slate-200 rounded-2xl shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input type="password" dir="ltr" className="w-full px-4 py-2 text-left border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all" value={password} onChange={(e) => setPassword(e.target.value)} required />
        </div>
      ...
      <div className="mt-6 text-center text-sm text-slate-600">
        עדיין אין לך חשבון? <Link to="/register" className="text-indigo-600 hover:underline font-medium">הרשם כאן</Link>
      </div>
    </div>
  );
};
