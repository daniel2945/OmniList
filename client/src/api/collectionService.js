...
export const addItemToCollection = async (collectionId, mediaItemId) => {
  return await API_CALL(`/collections/${collectionId}/add`, "POST", {
    mediaItemId,
  });
};
...
export const removeMediaFromCollection = async (collectionId, mediaId) => {
  return await API_CALL(
    `/collections/${collectionId}/item/${mediaId}`,
    "DELETE",
  );
};

export const updateCollectionOrder = async (collectionId, orderedMediaIds) => {
  // orderedMediaIds צריך להיות מערך של מזהים סטנדרטיים
  // התיקון: שימוש ב-API_CALL במקום ב-axios
  return await API_CALL(`/collections/${collectionId}/order`, "PUT", {
    orderedMediaIds,
  });
};
